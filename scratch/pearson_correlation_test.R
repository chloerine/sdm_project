...
cor_pairs_sub <- subset(x = cor_pairs, subset = (cor_pairs$correlation > 0.7) & (cor_pairs$var1 != cor_pairs$var2)) #only getting those with correaltion higher thatn 0.75 and those with same pair eg bio1 and bio1 wld have a correlation of 1
...
#only getting the upper right triangle of matrix, excluding diagonal, all other below are mirrored
p_cor_right <-p_cor_abs[ upper.tri(x = p_cor_abs, diag = FALSE)]

#creating a combination of a certain size from all the bioclim variables
all_subsets <- list()
for (i in 1:length(layer_names)) {
  subsets_i <- combn(x = layer_names, m = i, simplify = FALSE)  # returns a list of combinations of size i
  ...
  for (member_one in 1:length(current_subset)){ # getting first member of the pair
    for (member_two in member_one:length(current_subset)){ #getting second member of the pair
      if(all_subsets[[subset_index]][member_one] != all_subsets[[subset_index]][member_two]){ # making sure the members are not the same eg bio1 and boi1 are the pairs
        ...
        if(p_cor_abs[row_index,col_index] > 0.7){ #if correlation value is to high set the flag to false and break out of loop 
          valid_subset <- FALSE
